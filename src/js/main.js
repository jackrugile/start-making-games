...
function applyScale() {
  ...
  slideScale =
    Math.max(Math.min(slideScale, slideScaleMax), slideScaleMin) + 0.001;
  ...
  slideRequest.onload = function () {
    if (slideRequest.status >= 200 && slideRequest.status < 400) {
      ...
      slideTimeout = setTimeout(function () {
        if (lastSlide !== null) {
          // remove the old css
          var oldStylesheet = document.querySelector(
            ".stylesheet-" + lastSlide
          );
          if (oldStylesheet) {
...
var pg = playground({
  ...
  keydown: function (e) {
    if (keyMap.up.indexOf(e.key) > -1) {
      keys.up = 1;
      dirs.up.classList.add("is-active");
      window.dispatchEvent(controlUpDownEvent);
    }
    //else if( keyMap.right.indexOf( e.key ) > -1 ) { keys.right = 1; dirs.right.classList.add( 'is-active' ); }
    else if (keyMap.down.indexOf(e.key) > -1) {
      keys.down = 1;
      dirs.down.classList.add("is-active");
      window.dispatchEvent(controlDownDownEvent);
    }
    //else if( keyMap.left.indexOf( e.key ) > -1 )  { keys.left = 1; dirs.left.classList.add( 'is-active' ); }
    else if (keyMap.spike.indexOf(e.key) > -1) {
      window.dispatchEvent(controlSpikeDownEvent);
    } else if (keyMap.prev.indexOf(e.key) > -1) {
      prevSlide(e.original.shiftKey);
    } else if (keyMap.next.indexOf(e.key) > -1) {
      nextSlide(e.original.shiftKey);
    } else if (keyMap.mute.indexOf(e.key) > -1) {
      window.dispatchEvent(controlMuteDownEvent);
    } else if (keyMap.pause.indexOf(e.key) > -1) {
      window.dispatchEvent(controlPauseDownEvent);
    } else if (keyMap.zoom.indexOf(e.key) > -1) {
      toggleZoom();
    }
  },
  keyup: function (e) {
    if (keyMap.up.indexOf(e.key) > -1) {
      keys.up = 0;
      dirs.up.classList.remove("is-active");
      window.dispatchEvent(controlUpUpEvent);
    }
    //else if( keyMap.right.indexOf( e.key ) > -1 ) { keys.right = 0; dirs.right.classList.remove( 'is-active' ); }
    else if (keyMap.down.indexOf(e.key) > -1) {
      keys.down = 0;
      dirs.down.classList.remove("is-active");
      window.dispatchEvent(controlDownUpEvent);
    }
    //else if( keyMap.left.indexOf( e.key ) > -1 )  { keys.left = 0; dirs.left.classList.remove( 'is-active' ); }
    else if (keyMap.spike.indexOf(e.key) > -1) {
      window.dispatchEvent(controlSpikeUpEvent);
    }
  },
  gamepaddown: function (e) {
    if (keyMap.up.indexOf(e.button) > -1) {
      keys.up = 1;
      dirs.up.classList.add("is-active");
      window.dispatchEvent(controlUpDownEvent);
    } else if (keyMap.right.indexOf(e.button) > -1) {
      keys.right = 1;
      dirs.right.classList.add("is-active");
    } else if (keyMap.down.indexOf(e.button) > -1) {
      keys.down = 1;
      dirs.down.classList.add("is-active");
      window.dispatchEvent(controlDownDownEvent);
    } else if (keyMap.left.indexOf(e.button) > -1) {
      keys.left = 1;
      dirs.left.classList.add("is-active");
    } else if (keyMap.spike.indexOf(e.button) > -1) {
      window.dispatchEvent(controlSpikeDownEvent);
    } else if (keyMap.prev.indexOf(e.button) > -1) {
      prevSlide(this.gamepads[0].buttons["3"]);
    } else if (keyMap.next.indexOf(e.button) > -1) {
      nextSlide(this.gamepads[0].buttons["3"]);
    } else if (keyMap.mute.indexOf(e.button) > -1) {
      window.dispatchEvent(controlMuteDownEvent);
    } else if (keyMap.pause.indexOf(e.button) > -1) {
      window.dispatchEvent(controlPauseDownEvent);
    } else if (keyMap.refresh.indexOf(e.button) > -1) {
      this.refreshTick++;
      if (this.refreshTick > 1) {
        location.reload(true);
      }
    } else if (keyMap.zoom.indexOf(e.button) > -1) {
      toggleZoom();
    }
  },
  gamepadup: function (e) {
    if (keyMap.up.indexOf(e.button) > -1) {
      ...
      keys.right = 0;
      dirs.right.classList.remove("is-active");
    } else if (keyMap.down.indexOf(e.button) > -1) {
      keys.down = 0;
      dirs.down.classList.remove("is-active");
      window.dispatchEvent(controlDownUpEvent);
      ...
      keys.left = 0;
      dirs.left.classList.remove("is-active");
    } else if (keyMap.spike.indexOf(e.button) > -1) {
      window.dispatchEvent(controlSpikeUpEvent);
    }
  },
