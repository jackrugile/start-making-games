...
function setScale(){slideScale=slideWrap.offsetWidth>slideWrap.offsetHeight/slideRatio?slideWrap.offsetHeight/slideRatio/slideWidth:slideWrap.offsetWidth*slideRatio/slideHeight,slideScale=Math.max(Math.min(slideScale,slideScaleMax),slideScaleMin)+.001,slide.style.transform="scale("+slideScale+")"}function onResize(e){setScale()}function loadSlide(e){if(!t){location.hash=e+1,slideIndicator.innerHTML=e+1,document.title=slides[e]+" // Start Making Games",game&&"function"==typeof game.destroy,game=null,document.documentElement.classList.add("loading");var t=new XMLHttpRequest;t.open("GET","slides/"+slides[e]+"/index.html",!0),t.onload=function(){t.status>=200&&t.status<400&&(clearTimeout(slideTimeout),slideTimeout=setTimeout(function(){if(null!==lastSlide){var i=document.querySelector(".stylesheet-"+lastSlide);i&&i.parentNode.removeChild(i);var s=document.querySelector(".script-"+lastSlide);s&&s.parentNode.removeChild(s)}var n=document.createElement("link");n.rel="stylesheet",n.href="slides/"+slides[e]+"/index.css",n.classList.add("stylesheet-"+e),document.getElementsByTagName("head")[0].appendChild(n),slideContent.innerHTML=t.responseText;var o=document.createElement("script");o.setAttribute("src","slides/"+slides[e]+"/index.js"),o.async=!0,o.classList.add("script-"+e),document.getElementsByTagName("head")[0].appendChild(o),document.documentElement.classList.remove("loading"),t=null,Prism.highlightAll()},20))},t.send()}}function prevSlide(){currentSlide>0&&!slideRequest&&(document.documentElement.classList.add("prev"),document.documentElement.classList.remove("next"),lastSlide=currentSlide,currentSlide--,loadSlide(currentSlide))}function nextSlide(){totalSlides-1>currentSlide&&!slideRequest&&(document.documentElement.classList.add("next"),document.documentElement.classList.remove("prev"),lastSlide=currentSlide,currentSlide++,loadSlide(currentSlide))}function rand(e,t){return Math.random()*(t-e)+e}function randInt(e,t){return Math.floor(e+Math.random()*(t-e+1))}var game=null;"?dev"===window.location.search&&document.documentElement.classList.add("dev");var slides=["intro-t-title","intro-t-jack","intro-t-quotes","build-c-test-html","build-c-test-css","build-cd-test-js","build-d-final","juice-d-3d","juice-d-final"],slideWrap=document.querySelector(".slide-wrap"),slide=document.querySelector(".slide"),slideContent=document.querySelector(".slide-content"),slideBcr=slide.getBoundingClientRect(),slideWidth=slideBcr.width,slideHeight=slideBcr.height,slideRatio=slideHeight/slideWidth,slideScaleMin=0,slideScaleMax=1/0,slideScale;addEventListener("resize",onResize),setScale();var slideTimeout=null,currentSlide=location.hash?location.hash.slice(1)-1:0,lastSlide=null,totalSlides=slides.length,prevSlideButton=document.querySelector(".nav-prev"),nextSlideButton=document.querySelector(".nav-next"),slideIndicator=document.querySelector(".slide-indicator"),slideRequest;currentSlide>totalSlides&&(currentSlide=0),prevSlideButton.addEventListener("click",function(e){e.preventDefault(),prevSlide()}),nextSlideButton.addEventListener("click",function(e){e.preventDefault(),nextSlide()}),loadSlide(currentSlide);var controlDownDownEvent=new Event("controlDownDown"),controlDownUpEvent=new Event("controlDownUp"),controlUpDownEvent=new Event("controlUpDown"),controlUpUpEvent=new Event("controlUpUp"),mouseLeftDownEvent=new Event("mouseLeftDown"),mouseLeftUpEvent=new Event("mouseLeftUp"),mouseRightDownEvent=new Event("mouseRightDown"),mouseRightUpEvent=new Event("mouseRightUp"),keyMap={up:["up","w"],right:["right","d"],down:["down","s"],left:["left","a"],prev:["openbracket","l2"],next:["closebraket","l1"]},keys={up:0,right:0,down:0,left:0},dirs={up:document.querySelector(".dir-up"),right:document.querySelector(".dir-right"),down:document.querySelector(".dir-down"),left:document.querySelector(".dir-left")},pg=playground({mousemove:function(e){this.mouseIdle&&document.documentElement.classList.remove("mouse-idle"),this.mouseIdle=!1,this.mouseIdleTick=this.mouseIdleTickMax},mousedown:function(e){"left"==e.button&&window.dispatchEvent(mouseLeftDownEvent),"right"==e.button&&window.dispatchEvent(mouseRightDownEvent)},mouseup:function(e){"left"==e.button&&window.dispatchEvent(mouseLeftUpEvent),"right"==e.button&&window.dispatchEvent(mouseRightUpEvent)},keydown:function(e){keyMap.up.indexOf(e.key)>-1?(keys.up=1,dirs.up.classList.add("is-active"),window.dispatchEvent(controlUpDownEvent)):keyMap.right.indexOf(e.key)>-1?(keys.right=1,dirs.right.classList.add("is-active")):keyMap.down.indexOf(e.key)>-1?(keys.down=1,dirs.down.classList.add("is-active"),window.dispatchEvent(controlDownDownEvent)):keyMap.left.indexOf(e.key)>-1?(keys.left=1,dirs.left.classList.add("is-active")):keyMap.prev.indexOf(e.key)>-1?prevSlide():keyMap.next.indexOf(e.key)>-1&&nextSlide()},keyup:function(e){keyMap.up.indexOf(e.key)>-1?(keys.up=0,dirs.up.classList.remove("is-active"),window.dispatchEvent(controlUpUpEvent)):keyMap.right.indexOf(e.key)>-1?(keys.right=0,dirs.right.classList.remove("is-active")):keyMap.down.indexOf(e.key)>-1?(keys.down=0,dirs.down.classList.remove("is-active"),window.dispatchEvent(controlDownUpEvent)):keyMap.left.indexOf(e.key)>-1&&(keys.left=0,dirs.left.classList.remove("is-active"))},gamepaddown:function(e){keyMap.up.indexOf(e.button)>-1?(keys.up=1,dirs.up.classList.add("is-active"),window.dispatchEvent(controlUpDownEvent)):keyMap.right.indexOf(e.button)>-1?(keys.right=1,dirs.right.classList.add("is-active")):keyMap.down.indexOf(e.button)>-1?(keys.down=1,dirs.down.classList.add("is-active"),window.dispatchEvent(controlDownDownEvent)):keyMap.left.indexOf(e.button)>-1?(keys.left=1,dirs.left.classList.add("is-active")):keyMap.prev.indexOf(e.button)>-1?prevSlide():keyMap.next.indexOf(e.button)>-1&&nextSlide()},gamepadup:function(e){keyMap.up.indexOf(e.button)>-1?(keys.up=0,dirs.up.classList.remove("is-active"),window.dispatchEvent(controlUpUpEvent)):keyMap.right.indexOf(e.button)>-1?(keys.right=0,dirs.right.classList.remove("is-active")):keyMap.down.indexOf(e.button)>-1?(keys.down=0,dirs.down.classList.remove("is-active"),window.dispatchEvent(controlDownUpEvent)):keyMap.left.indexOf(e.button)>-1&&(keys.left=0,dirs.left.classList.remove("is-active"))},paths:{sounds:"snd/"},create:function(){this.dt=.016,this.dtMs=16,this.dtNorm=1,this.time=0,this.timeMs=0,this.timeNorm=0,this.mouseIdle=!1,this.mouseIdleTickMax=180,this.mouseIdleTick=this.mouseIdleTickMax,this.loadSounds(["paddle-1","wall-1","score-player-1","score-enemy-1","spike-1","spike-2","spike-3","slow-mo-1"])},step:function(e){this.manageTime(e),this.mouseIdle||(this.mouseIdleTick>0?this.mouseIdleTick--:(this.mouseIdle=!0,document.documentElement.classList.add("mouse-idle")))},manageTime:function(e){this.dt=e,this.dtMs=1e3*this.dt,this.dtNorm=60*this.dt,this.time+=this.dt,this.timeMs+=this.dtMs,this.timeNorm+=this.dtNorm},getDt:function(){return void 0==this.dtNorm?1:this.dtNorm}}),G=function(e){this.config={paddle:{width:60,height:240,speed:16},ball:{width:60,height:60,speed:1,inc:1},score:{max:5}},this.raf=null,this.stage=new this.Stage(this),this.ball=new this.Ball(this),this.paddlePlayer=new this.Paddle(this,!0),this.paddleEnemy=new this.Paddle(this,!1),this.scorePlayer=new this.Score(this,!0),this.scoreEnemy=new this.Score(this,!1),this.screenshake=new this.Screenshake(this),this.timescale={current:1,target:.2,timer:0,timerMax:180,inDuration:.6,outDuration:1,isSlow:!1}};G.prototype.Stage=function(e){this.g=e,this.elem=document.querySelector(".pong"),this.xDeg=0,this.xDegTarget=0,this.yDeg=0,this.yDegTarget=0,this.rangeDeg=10,this.xTrans=0,this.xTransTarget=0,this.yTrans=0,this.yTransTarget=0,this.smoothingDeg=.05,this.rotation=0,this.scaleBase=.85,this.scale=this.scaleBase,this.width=1920,this.height=1080},G.prototype.Stage.step=function(){this.xDegTarget=2*(g.ball.y/(this.height-g.ball.height)-.5)*this.rangeDeg,this.xDeg+=(this.xDegTarget-this.xDeg)*this.smoothingDeg,this.yDegTarget=2*(g.ball.x/(this.width-g.ball.width)-.5)*this.rangeDeg,this.yDeg+=(this.yDegTarget-this.yDeg)*this.smoothingDeg,this.xTransTarget=.1*(-g.ball.x+this.width/2)*(10*(1-g.timescale.current))+shake.x,this.xTrans+=(this.xTransTarget-this.xTrans)*this.smoothingDeg,this.yTransTarget=.1*(-g.ball.y+this.height/2)*(10*(1-g.timescale.current))+shake.y,this.yTrans+=(this.yTransTarget-this.yTrans)*this.smoothingDeg,this.rotation=shake.angle,this.scale=this.scaleBase+.3*(1-g.timescale.current)},G.prototype.Stage.draw=function(){this.style.transform="scale( "+this.scale+" ) translateX("+this.xTrans+"px) translateY("+this.yTrans+"px) rotateX("+-this.xDeg+"deg) rotateY("+this.yDeg+"deg) rotateZ("+this.rotation+"rad)"},G.prototype.Ball=function(e){this.g=e,this.elem=document.querySelector(".ball"),this.serving=!0,this.servingTimer=0,this.servingTimerMax=60,this.x=e.stage.width/2-e.config.ball.width/2,this.y=e.stage.height/2-e.config.ball.height/2,this.z=60,this.vx=e.config.ball.speed,this.vy=e.config.ball.speed,this.width=e.config.ball.width,this.height=e.config.ball.height,this.rotation=Math.PI/4},G.prototype.reset=function(){this.ball.serving=!0,this.ball.x=this.stage.width/2-this.ball.width/2,this.ball.y=this.stage.height/2-this.ball.height/2,this.ball.vx=0,this.ball.vy=0},G.prototype.Ball.step=function(){this.serving?(this.servingTimer<this.servingTimerMax?this.servingTimer++:(this.serving=!1,this.servingTimer=0),this.ball.rotation=Math.PI/4):ball.vx>0?this.rotation+=.2*g.getDt():this.rotation-=.2*g.getDt()},G.prototype.Ball.draw=function(){this.elem.style.transform="translate3d("+this.x+"px, "+this.y+"px, "+this.z+"px) rotateZ("+this.rotation+"rad)"},G.prototype.Paddle=function(e,t){this.g=e,this.isPlayer=t,this.isEnemy=!t,this.y=e.stage.height/2-e.config.paddle.height/2,this.z=60,this.width=e.config.paddle.width,this.height=e.config.paddle.height,this.speed=e.config.paddle.speed,this.isPlayer?(this.elem=document.querySelector(".paddle-player"),this.x=0):(this.elem=document.querySelector(".paddle-enemy"),this.x=e.stage.width-e.config.paddle.width)},G.prototype.Paddle.step=function(){},G.prototype.Paddle.draw=function(){this.elem.style.transform="translate3d("+this.x+"px, "+this.y+"px, "+this.z+"px)"},G.prototype.Score=function(e,t){this.g=e,this.isPlayer=t,this.isEnemy=!t,this.value=0,this.flag=!0,this.isPlayer?this.elem=document.querySelector(".score-player"):this.elem=document.querySelector(".score-enemy")},G.prototype.Score.step=function(){},G.prototype.Score.draw=function(){this.flag&&(this.elem.innerHTML=this.value,this.elem.setAttribute("data-score",this.value),this.flag=!1)},G.prototype.Screenshake=function(e){this.g=e,this.translate=0,this.rotate=0,this.x=0,this.y=0,this.xTarget=0,this.yTarget=0,this.xBias=0,this.yBias=0,this.angle=0,this.angleTarget=0},G.prototype.Screenshake.step=function(){this.xBias*=.9,this.yBias*=.9,this.translate>0?(this.translate*=.9,this.xTarget=rand(-this.translate,this.translate)+this.xBias,this.yTarget=rand(-this.translate,this.translate)+this.yBias):(this.xTarget=0,this.yTarget=0),this.rotate>0?(this.rotate*=.9,this.angleTarget=rand(-this.rotate,this.rotate)):this.angleTarget=0,this.x+=.1*(this.xTarget-this.x),this.y+=.1*(this.yTarget-this.y),this.angle+=.1*(this.angleTarget-this.angle)},G.prototype.Screenshake.draw=function(){},G.prototype.init=function(){this.addEventListeners(),this.loop()},G.prototype.reset=function(){this.scorePlayer.value=0,this.scoreEnemy.value=0,this.ball.speed=this.config.ball.speed,this.ball.reset()},G.prototype.addEventListeners=function(){window.addEventListener("controlUpDown",onControlUpDown),window.addEventListener("controlDownDown",onControlDownDown),window.addEventListener("controlUpUp",onControlUpUp),window.addEventListener("controlDownUp",onControlDownUp),window.addEventListener("mouseLeftDown",onMouseLeftDown),window.addEventListener("mouseRightDown",onMouseRightDown),window.addEventListener("mouseLeftUp",onMouseLeftUp),window.addEventListener("mouseRightUp",onMouseRightUp)},G.prototype.removeEventListeners=function(){window.removeEventListener("controlUpDown",onControlUpDown),window.removeEventListener("controlDownDown",onControlDownDown),window.removeEventListener("controlUpUp",onControlUpUp),window.removeEventListener("controlDownUp",onControlDownUp),window.removeEventListener("mouseLeftDown",onMouseLeftDown),window.removeEventListener("mouseRightDown",onMouseRightDown),window.removeEventListener("mouseLeftUp",onMouseLeftUp),window.removeEventListener("mouseRightUp",onMouseRightUp)},G.prototype.onControlUpDown=function(){},G.prototype.onControlDownDown=function(){},G.prototype.onControlUpUp=function(){},G.prototype.onControlDownUp=function(){},G.prototype.onMouseLeftDown=function(){slowMo()},G.prototype.onMouseRightDown=function(){},G.prototype.onMouseLeftUp=function(){},G.prototype.onMouseRightUp=function(){},G.prototype.collisionAABB=function(e,t){return e.x>t.x+t.width||e.x+e.width<t.x||e.y>t.y+t.height||e.y+e.height<t.y?void 0:!0},G.prototype.step=function(){},G.prototype.draw=function(){this.stage.draw(),this.paddlePlayer.draw(),this.paddleEnemy.draw(),this.ball.draw(),this.scorePlayer.draw(),this.scoreEnemy.draw()},G.prototype.loop=function(){this.raf=requestAnimationFrame(this.loop.bind(this)),this.step(),this.draw()},G.prototype.kill=function(){this.removeEventListeners(),cancelAnimationFrame(this.raf)};